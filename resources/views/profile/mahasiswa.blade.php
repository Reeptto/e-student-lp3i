        ...
        <div class="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-12 mt-8">
            ...
            <div class="lg:col-span-4 space-y-8">
                <div class="pop-card">
                    ...
                    <div class="pop-inner p-6 flex flex-col items-center text-center bg-white/50 backdrop-blur-sm">
            ...
            <div class="lg:col-span-8">
                <div class="pop-card h-full">
                    ...
                    <div class="pop-inner p-8 pt-12 bg-white/80 backdrop-blur-sm">
                        ...
                        {{-- VIEW MODE --}}
                        <div x-show="!isEditing" class="h-full flex flex-col">
                            
                            <div class="flex flex-wrap gap-3 mb-8 justify-center md:justify-start">
                                <button @click="activeTab = 'akademik'" :class="activeTab === 'akademik' ? 'active' : ''" class="pop-tab">Akademik</button>
                                <button @click="activeTab = 'pribadi'" :class="activeTab === 'pribadi' ? 'active' : ''" class="pop-tab">Pribadi</button>
                                <button @click="activeTab = 'kontak'" :class="activeTab === 'kontak' ? 'active' : ''" class="pop-tab">Kontak</button>
                            </div>

                            <div class="bg-white rounded-[2rem] border-[3px] border-[#004269] p-8 shadow-sm flex-grow relative overflow-hidden">
                                <div class="absolute top-0 right-0 w-20 h-20 bg-[#009DA5] opacity-10 rounded-bl-[3rem]"></div>

                                {{-- Tab Akademik --}}
                                <div x-show="activeTab === 'akademik'" class="space-y-6 relative z-10">
                                    <div class="flex items-center gap-3 mb-6">
                                        <div class="bg-[#009da5]

 text-[#004269] w-10 h-10 rounded-xl flex items-center justify-center border-2 border-[#004269] shadow-[2px_2px_0_#004269]"><i class="fas fa-star text-xl"></i></div>
                                        <h3 class="text-2xl font-black text-[#004269] uppercase italic">ACADEMIC INFO</h3>
                                    </div>
                                    
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div class="info-bubble">
                                            <span class="info-bubble-label">KELAS SAAT INI</span>
                                            <p class="font-black text-xl text-[#004269]">{{ $mahasiswa->kelas->kode_kelas }}</p>
                                        </div>
                                        <div class="info-bubble">
                                            <span class="info-bubble-label">STATUS MAHASISWA</span>
                                            <div class="flex items-center gap-2">
                                                <span class="w-3 h-3 bg-emerald-500 rounded-full"></span>
                                                <p class="font-black text-xl text-emerald-500">Aktif</p>
                                            </div>
                                        </div>
                                        <div class="info-bubble md:col-span-2">
                                            <span class="info-bubble-label">Bidang Keahlian</span>
                                            <p class="font-black text-xl text-[#004269]">{{ $mahasiswa->bidangKeahlian?->nama_bidang_keahlian ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>

                                {{-- Tab Pribadi --}}
                                <div x-show="activeTab === 'pribadi'" x-cloak class="space-y-6 relative z-10">
                                    <div class="flex items-center gap-3 mb-6">
                                        <div class="bg-bg-[#f15b67]
 text-[#004269] w-10 h-10 rounded-xl flex items-center justify-center border-2 border-[#004269] shadow-[2px_2px_0_#004269]"><i class="fas fa-heart text-xl"></i></div>
                                        <h3 class="text-2xl font-black text-[#004269] uppercase italic">PERSONAL INFO</h3>
                                    </div>

                                    <div class="space-y-5">
                                        <div class="info-bubble">
                                            <span class="info-bubble-label">NAMA LENGKAP</span>
                                            <p class="font-black text-xl text-[#004269]">{{ $mahasiswa->nama_mhs }}</p>
                                        </div>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            <div class="info-bubble">
                                                <span class="info-bubble-label"><i class="fas fa-map-marker-alt mr-1"></i> TEMPAT LAHIR</span>
                                                <p class="font-bold text-lg text-[#004269]">{{ $mahasiswa->tempat_lahir }}</p>
                                            </div>
                                            <div class="info-bubble">
                                                <span class="info-bubble-label"><i class="fas fa-calendar-alt mr-1"></i> TANGGAL LAHIR</span>
                                                <p class="font-bold text-lg text-[#004269]">{{ $mahasiswa->tanggal_lahir }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {{-- Tab Kontak --}}
                                <div x-show="activeTab === 'kontak'" x-cloak class="space-y-6 relative z-10">
                                    <div class="flex items-center gap-3 mb-6">
                                        <div class="bg-[#ffffff]
 text-[#004269] w-10 h-10 rounded-xl flex items-center justify-center border-2 border-[#004269] shadow-[2px_2px_0_#004269]"><i class="fas fa-address-book text-xl"></i></div>
                                        <h3 class="text-2xl font-black text-[#004269] uppercase italic">Kontak Mahasiswa</h3>
                                    </div>

                                    <div class="info-bubble flex items-center justify-between group hover:border-[#004269] transition">
                                        <div>
                                            <span class="info-bubble-label">ALAMAT EMAIL</span>
                                            <p class="font-bold text-lg text-[#004269]">{{ $mahasiswa->email }}</p>
                                        </div>
                                        <div class="bg-blue-100 p-3 rounded-full text-blue-600 border-2 border-blue-200"><i class="fas fa-envelope"></i></div>
                                    </div>
                                    <div class="info-bubble flex items-center justify-between group hover:border-[#004269] transition">
                                        <div>
                                            <span class="info-bubble-label">NOMOR TELEPONE</span>
                                            <p class="font-bold text-lg text-[#004269]">{{ $mahasiswa->no_telp }}</p>
                                        </div>
                                        <div class="bg-green-100 p-3 rounded-full text-green-600 border-2 border-green-200"><i class="fas fa-phone"></i></div>
                                    </div>
                                    <div class="info-bubble group hover:border-[#004269] transition">
                                        <span class="info-bubble-label">DOMISILI SAAT INI</span>
                                        <p class="font-bold text-lg text-[#004269] leading-relaxed mt-1">{{ $mahasiswa->Domisili }}</p>
                                    </div>
                                </div>
